...
*Không phải game nào cũng sẽ tương thích với MKXP. Hãy chuẩn bị cho việc game bạn muốn chơi có thể sẽ không chơi được bằng phần mềm này.<br>Chỉ hỗ trợ RPG Maker XP/VX/VX Ace. Các phần mềm làm game khác sẽ không được hỗ trợ.*
...
Lưu ý là không phải game nào cũng có thể chạy được bằng cách này.
...
* Sao chép tệp tin `x86_64-pc-windows-msvc-simple-http-server.exe` vừa mới tải xuống vào thư mục `www` của game (hoặc thư mục chính của game nếu không thấy thư mục `www`).<br>*Bạn có thể đổi tên tệp tin `.exe` kia thành tên dễ nhớ hơn.*
...
* Sao chép địa chỉ ở phần **Address** của cửa sổ đó, như ảnh bên trên là `http://0.0.0.0:8000` và dán vào trình duyệt. Giao diện như thế này sẽ xuất hiện.

![88](images/image-87.png)

* Đợi cho game tải tài nguyên và vào màn hình menu như lúc bạn chạy bằng tệp `Game.exe`. Khi bạn không chơi game nữa thì có thể tắt cửa sổ đi.